...
%DIF DEL ../economics-open-science-old/article.tex                            Sun Jul 13 20:21:12 2025
%DIF ADD /home/vilhuber/Workspace/Github/economics-open-science/article.tex   Wed Jul 16 22:24:05 2025
...
Data sharing was harder, of course, with lots of floppies\footnote{\url{https://en.wikipedia.org/wiki/Floppy_disk}} being exchanged, but also the use of departmental FTP\footnote{\url{https://en.wikipedia.org/wiki/File_Transfer_Protocol}} servers (David Card's collection of data, or the NBER's), or even the replication archive of the Journal of Applied Econometrics, instantiated at Queens University in 1994 under the long-running guidance of James McKinnon.\footnote{The JAE archive was migrated to the ZBW's archives in 2022 and can now be found at \url{https://journaldata.zbw.eu/journals/jae}, but legacy files are still visible as of 2024 at \url{http://qed.econ.queensu.ca/jae/legacy.html}.} \DIFdelbegin \DIFdel{. }\DIFdelend On the other hand, administrative data, such as the French administrative data used in AKM required travel to Paris, sitting in a room without windows at an assigned time, and typing the code into the system that had access.\DIFaddbegin \footnote{\DIFaddend For non-local \DIFdelbegin \DIFdel{authors, this meant traveling for extended time periods as a Ph.D. student (Margolis) or spending a sabbatical in Paris (Abowd), neither of which is a cheap endeavor.}%DIFDELCMD < \footnote{%%%
\DIFdelend \DIFaddbegin \DIFadd{authors, this meant traveling for extended time periods as a Ph.D. student (Margolis) or spending a sabbatical in Paris (Abowd), neither of which is a cheap endeavor. }\DIFaddend Both were, and still are, enjoyable, though.}
...
In order to write about ``Open Science,'' a definition is needed. Open science is a surprisingly difficult term to define precisely\DIFaddbegin \DIFadd{, and multiple overlapping definition are commonly referenced}\DIFaddend . 
%
...
\DIFaddbegin \noindent \DIFaddend The Open Knowledge Foundation \parencite[OKF]{open_knowledge_foundation_defining_2024} defines ``open'' as (my emphasis)
...
\paragraph{Organic data:} However, most economists, when asked about data subject to restrictions, will think of ``proprietary data,'' a term often applied to any data that may be subject to restrictions of use and re-use. Access to most administrative data is typically not ``open'' in the sense of \DIFdelbegin \DIFdel{OKD}\DIFdelend \DIFaddbegin \DIFadd{OKF}\DIFaddend , but are they open enough, given the privacy concerns that are attached to these data? Many have argued that access is not broad enough \citep{card_expanding_2010,einav_economics_2014}, while acknowledging the difficulty of addressing privacy and security of the data at scale. \citet{abowd_economic_2018} discuss the challenge of making the choice of between accuracy of (public) statistics and data, and the privacy loss inherent in doing so. \citet{nagaraj_improving_2020} argue that more openness improves scientific progress, and \citet{nagaraj_how_2023} study this in the context of the US system for providing access (\ac{FSRDC}). They note that 4\% of US-based empirical authors have had some access to the FSRDC system. I am not aware of similar studies for other countries, such as France \citep[the equivalent system is the \ac{CASD}, ][]{gadouche_centre_2019} or Canada \citep{currie2015social}. In absolute terms, these networks host several hundred researchers every year. \DIFdelbegin \DIFdel{As per (CENSUS)}\DIFdelend \DIFaddbegin \DIFadd{For the 781 projects using Census Bureau data in the }\ac{FSRDC}\DIFaddend , 2,084 researchers  have had access to \DIFdelbegin \DIFdel{projects involving Census Bureau data since (YEAR).}\DIFdelend \DIFaddbegin \DIFadd{confidential data between 1998 and 2024.}\DIFaddend \footnote{\DIFaddbegin \DIFadd{Own calculations based on Census Bureau data, see }\DIFaddend \url{https://labordynamicsinstitute.github.io/fsrdc-external-census-projects/}\DIFaddbegin \DIFadd{.}\DIFaddend } \citet{nagaraj_how_2023} mention 861 papers \DIFaddbegin \DIFadd{in scientific journals}\DIFaddend . Similar numbers can be obtained for the French (\DIFdelbegin \DIFdel{xxx researchers and }\DIFdelend \DIFaddbegin \DIFadd{6841 researchers from 1109 institutions on 1797 projects with }\DIFaddend 417 publications)\DIFaddbegin \footnote{\DIFadd{From }\url{https://www.casd.eu/} \DIFadd{and }\url{https://www.casd.eu/toutes-les-publications/liste/0/20/} \DIFadd{as of May 2025.}}
\DIFaddend % https://www.casd.eu/toutes-les-publications/liste/0/20/ 
% 20 pages à 20 publications et 1 à 17 = 417
and Canadian (2201 active researchers \DIFaddbegin \DIFadd{from 42 universities }\DIFaddend as of January 2025, \DIFdelbegin \DIFdel{and }\DIFdelend \DIFaddbegin \DIFadd{with }\DIFaddend 3245 papers published between 2000 and May 2024)\footnote{Provided by Grant Gibson, CRDCN, on February 10, 2025.} networks.\footnote{The \citet{nagaraj_how_2023} number only includes papers published by economists. \DIFdelbegin \DIFdel{All other }\DIFdelend \DIFaddbegin \DIFadd{Other }\DIFaddend numbers are counts of researchers and publications in all disciplines, \DIFaddbegin \DIFadd{in non-peer-reviewed publications, }\DIFaddend and include non-economists.} The number of publications from access to these networks is smaller in absolute terms then those from PSID and SOEP, though likely higher in impact \citep{nagaraj_how_2023}.\DIFaddbegin \footnote{\DIFadd{For an analysis of code availability over time for SOEP-based publications, see \mbox{%DIFAUXCMD
\citet{fink_replication_2025}}\hskip0pt%DIFAUXCMD
.}}
\DIFaddend 
...
`Providing access to datasets and data management tools' as one of the two factors. University staff and students (across multiple disciplines) were also asked which aspect they  most needed funding for, and 38\% mentioned `Data analysis software '.}\DIFdelend %DIF >  new version
\DIFaddbegin \DIFadd{55\% of African scholars noted that providing access to research resources, such as journals, datasets, and software would improve their journal acceptance rate.%DIF > 
%DIF > 
%DIF >  Previously:
%DIF > 46\% of respondents chose 
%DIF > `Providing access to datasets and data management tools' as one of the two factors. University staff and students (across multiple disciplines) were also asked which aspect they  most needed funding for, and 38\% mentioned `Data analysis software' \citep{sheth_understanding_nodate}. 
}\DIFaddend Within top universities, on the other hand, most researchers, including graduate students, will have access to these software products, and even many undergraduates may be able to leverage university computer labs to access these software. \DIFdelbegin \DIFdel{Outside }\DIFdelend \DIFaddbegin \DIFadd{At lower-ranking institutions and outside }\DIFaddend of academia, however, it may be more constraining\DIFaddbegin \DIFadd{, even in Europe or the United States}\DIFaddend . Precise information is hard to come by\DIFdelbegin \DIFdel{.}\DIFdelend \DIFaddbegin \DIFadd{, but  Figure~\ref{fig:downloads} provides some insights whether this might, in fact, hold. The data plotted relates, by various regions, the relative importance of that region in global Stata and R downloads.}\footnote{\DIFadd{See Appendix for a detailed explanation of several important caveats.}}\DIFadd{. The top panel summarizes this by ``Global North'' and ``South,'' and the juxtaposition would suggest that the countries of the Global South are heavier users of the free R software, rather than the paid-for Stata software. However, the bottom panel moderates that view somewhat when breaking this out by continents. It turns out that the richest continent (North America) is the heaviest R user, and that Africa is a heavier user of Stata. Thus, there is, at best, very weak evidence that the cost of software matters, in the aggregate.  
}
...
\DIFdelbegin \DIFdel{To illustrate some of the benefits of }\DIFdelend \DIFaddbegin \DIFadd{Tthe discussion about benefits of open science often centers around }\textit{\DIFadd{data}} \DIFadd{availability. The World Bank, in its annual World Development Report, identifies data availability --- for research, for commerce, for education --- as a key contributor, and highlights that many LMIC continue to have impediments to the reliable provision of open access data \mbox{%DIFAUXCMD
\citep[, pg. 62]{world_bank_world_2021}}\hskip0pt%DIFAUXCMD
. The (theoretical) optimal level of data availability intersects with privacy, making the optimal level of data availability a non-trivial balance between public and private benefits, and private costs \mbox{%DIFAUXCMD
\citep{duch-brown_economics_2017,acquisti_economics_2016,abowd_suboptimal_2019,abowd_economic_2018}}\hskip0pt%DIFAUXCMD
. The more recent discussions (and court cases) surrounding the use of data in the training of large language models have only re-emphasized this tension \mbox{%DIFAUXCMD
\citep{panettieri_generative_2025}}\hskip0pt%DIFAUXCMD
.
}

\DIFadd{A different thread in the discussion brings up normative reasons for transparency, for instance around Mertonian \mbox{%DIFAUXCMD
\citep{merton1942note} }\hskip0pt%DIFAUXCMD
norms of openness \mbox{%DIFAUXCMD
\citep[see ][for an overview]{miguel_evidence_2021}}\hskip0pt%DIFAUXCMD
. Openness at all stages of research may act as a moderator for publication bias \mbox{%DIFAUXCMD
\citep{miguel_evidence_2021,brodeur_star_2016}
}\hskip0pt%DIFAUXCMD
}

\DIFadd{Some of the  benefits of open science have  been measured indirectly, through increased citations, say. Some recent studies find some advantages for studies with linked (openly available) data \mbox{%DIFAUXCMD
\citep{piwowar_data_2013,colavizza_citation_2020,christensen_study_2019}}\hskip0pt%DIFAUXCMD
.}\footnote{\DIFadd{Some of these studies need to be taken with a grain of salt, since they typically measure whether data is referenced, not whether it is actually openly available.}} \DIFadd{The economics literature has emphasized the benefits of (balanced) open science. \mbox{%DIFAUXCMD
\citet{nagaraj_improving_2020} }\hskip0pt%DIFAUXCMD
discuss the canonical example of improved data access through (free) public-use data in the context of satellite imagery. Patents can be usefully investigated, since they are both openly viewable but also access-limiting by their very nature. Economists have looked at how restrictiveness, duration, and type of patents affect scientific progress. In general, restrictions reduce social welfare \mbox{%DIFAUXCMD
\citep{williams_intellectual_2013,murray_mice_2016}}\hskip0pt%DIFAUXCMD
, even enabling anti-competitive behavior that directly identify welfare loss \mbox{%DIFAUXCMD
\citep{xie_anticompetitive_2020}}\hskip0pt%DIFAUXCMD
. The much broader applicability of open science practices is also much more recent, and as of yet, hard to measure. Nevertheless, it appears to be widely accepted in economics, as evidenced by very strong positive attitudes documented in \mbox{%DIFAUXCMD
\citet{ferguson_survey_2023}}\hskip0pt%DIFAUXCMD
, as selectively depicted in  Figure~\ref{fig:christensen-fig1b}. The top left part of Figure~\ref{fig:christensen-fig1b} shows behavior (black) and opinions (color scale) in regards to data sharing among economists, with more than 50\% of economists having shared data, but over 90\% being very much or moderately (the two green colors) in favor of sharing data. The right panel illustrates the evolution over time, depicting the proportion of social scientists in the four disciplines (economics, political science, psychology, and sociology) who had adopted an open science practice as of a particular date, showing again the rapid increase in active data sharing amongst economists from around 60\% in 2011 to the 2020 number of over 90\%.  
}
...
\DIFdelbegin \DIFdel{In both }\DIFdelend \DIFaddbegin \DIFadd{Finally, \mbox{%DIFAUXCMD
\citet{goldsmith-pinkham_contamination_2024} }\hskip0pt%DIFAUXCMD
investigate contamination bias (``each treatment’s effect are contaminated by nonconvex averages of the effects of other treatments''). To do so, they search the centralized repository of AEA replication packages}\footnote{\DIFadd{``These studies were identified by a systematic search of papers in the AEA Data and Code Repository.'' }[\DIFadd{pg. 4043}]} \DIFadd{to identify  packages that, crucially, contain data. They then reproduce one of each original paper's specifications, conduct several tests, and conclude that ``economically and statistically meaningful contamination bias }[\DIFadd{is present}] \DIFadd{in two of the three observational studies while showing no evidence for bias in any of the experimental studies.'' (pg. 4046). \mbox{%DIFAUXCMD
\citet{goldsmith-pinkham_contamination_2024}  }\hskip0pt%DIFAUXCMD
has been cited between 65 and 118 (OA, GS) times.}\footnote{\DIFadd{Google Scholar citations are directly reported from a view of the article's page on Google Scholar, and may include citations to multiple versions. OpenAlex citations are the sum of citations to all recorded works on OA with the same title and by the same authors.}}

\DIFadd{In each }\DIFaddend of these examples, the ability to access prior data, code, and information is critical to improving future scientific progress\DIFdelbegin \DIFdel{, but is }\DIFdelend \DIFaddbegin \DIFadd{. In some cases, it remains }\DIFaddend limited by both historical and unavoidable limitations on openness\DIFaddbegin \DIFadd{, as well as scaling limitation based on absence of ``push button'' reproducibility}\DIFaddend .
...
%DIFDELCMD <     %%%
\DIFdel{``These studies were identified by a systematic search of papers in the AEA Data and Code Repository''
}%DIFDELCMD < \end{quote}
%DIFDELCMD < %%%
\DIFdelend 
...
\DIFdelbegin \DIFdel{At the time of this writing}\DIFdelend \DIFaddbegin \DIFadd{Over the past eight years}\DIFaddend , I have \DIFaddbegin \DIFadd{or have had }\DIFaddend three editorial appointments. I am the Data Editor for the journals of the \acf{AEA} \DIFdelbegin \DIFdel{, the }\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{10.1257/pandp.108.745}}\hskip0pt%DIFAUXCMD
,  a column editor for the open access }\ac{HDSR} \DIFadd{\mbox{%DIFAUXCMD
\citep{vilhuberReinforcingReproducibilityReplicability2023}}\hskip0pt%DIFAUXCMD
, and until January 2025, the }\DIFaddend joint executive editor for the open access and multi-disciplinary \ac{JPC}, \DIFdelbegin \DIFdel{and I am a column editor for the open access }%DIFDELCMD < \ac{HDSR}%%%
\DIFdelend \DIFaddbegin \DIFadd{for which I continue to manage the publication infrastructure \mbox{%DIFAUXCMD
\citep{abowd_changes_2025}}\hskip0pt%DIFAUXCMD
}\DIFaddend . I will use each of these to highlight a particular pattern in broadening access to publications, without any claim to generality. 
...
\subsection*{\DIFadd{Inferring Stata and R usage}}

\DIFadd{Stata usage is inferred from downloads of Stata packages from the SSC web server, which is the sole official location to obtain these packages. Private mirrors may exist, and not all Stata packages are installed from SSC - both the Stata Journal and Github are likely to be significant sources. Data are obtained from log files from the SSC web server, provided by Kit Baum. R usage is inferred from downloads of  R binaries (for Windows and MacOS). While this is likely to be less frequent than Stata packages, relative patterns are of interest. There is no easy way to obtain the full list of downloads for all packages, other than cycling through several thousand such packages. The data stem from one of dozens of }\ac{CRAN} \DIFadd{mirrors, though this one, managed by Posit PBC, is the first one listed in the list of mirrors that are offered to users. Data in both cases is for February 2025.
}

\DIFadd{As a first step, these downloads were mapped to countries, and then aggregated by regions (Table~\ref{tab:stata_downloads_regional_stats_select} for Stata, Table~\ref{tab:r_downloads_regional_stats_select} for R).
%DIF > 
%DIF > 
}\input{outputs/stata_downloads_regional_stats_select}
\input{outputs/r_downloads_regional_stats_select}
%DIF > 
\DIFadd{Mapped onto a world map, this provides a pretty picture, though not very informative (Figure~\ref{fig:stata_r_world}).
}\begin{figure}
    \centering
    ...
    \caption{\DIFaddFL{Downloads of Stata packages and R software, by region, without China. Source: published data by Kit Baum and Posit PBC, own calculations.}}
...
\DIFadd{I then aggregate countries and regions into "Global North" and "Global South" (Table~\ref{tab:stata_downloads_global_stats_select_nochina} for Stata, Table~\ref{tab:r_downloads_global_stats_select_nochina} for R). 
%DIF > 
%DIF > 
...
%DIF > 
%DIF > 
\DIFadd{Differencing the two tables  gives the relative importance of Stata versus R, namely that there are relatively more downloads of Stata packages in the Global North than there are of R binaries, suggesting (very tentatively) that Stata usage may be higher in the Global North (Table~\ref{tab:ns_compare})
